    ...
    <?php foreach($infos as $info){?>
      ...
      <div class="text-truncate text-truncate--5 text-justify">
        ...
        <?php if($_SESSION['u_role'] == "admin"){?>
          <a href="asas" class="btn btn-secondary active text-white mr-1" 
          data-toggle="modal" 
          data-target="#modal<?php echo $info['ab_id']?>">Edit</a>
        <?php }?>
        <a href="single.php?id=<?php echo $info['ab_id']?>" class="btn btn-primary active text-white" target="_blank" rel="noopener noreferrer">Read More</a>
        <a href="single.php?id=<?php echo $info['ab_id']?>" class="btn btn-primary active text-white" target="_blank" rel="noopener noreferrer" style="visibility: hidden;">Read More</a>
        </div>
